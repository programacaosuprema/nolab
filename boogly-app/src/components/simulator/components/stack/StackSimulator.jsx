export class StackSimulator {
  ...
  empilhar(name, value) {
    ...
    if (limit > 0 && this.stacks[name].length >= limit) {
      this.steps.push({
        type: 'warning',
        message: `A pilha ${name} atingiu o tamanho máximo`,
        state: this.snapshot(),
      });
    ...
    this.steps.push({
      type: 'push',
      value,
      message: `Elemento ${value} entrou na pilha ${name}`,
      state: this.snapshot(),
    });
  }

  desempilhar(name) {
    if (!this.stacks[name] || this.stacks[name].length === 0) return;
  ...
  topo(name) {
    const value = this.stacks[name]?.[this.stacks[name].length - 1];
    ...
    this.steps.push({
      ...
      message: `Topo da pilha ${name}: ${value}`,
  ...
  tamanho_pilha(name) {
    const size = this.stacks[name]?.length || 0;
  ...
  pilha_vazia(name) {
    const empty = !this.stacks[name] || this.stacks[name].length === 0;
    ...
    this.steps.push({
      ...
      message: `Pilha ${name} ${empty ? 'está vazia' : 'não está vazia'}`,
