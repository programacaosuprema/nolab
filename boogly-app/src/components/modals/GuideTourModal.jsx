...
const defaultSteps = [
  {
    id: 'editor',
    title: 'Editor',
    content: 'Área de programação: arraste e conecte blocos aqui.',
  ...
  },
  {
    id: 'simulator',
    title: 'Simulador',
    content: 'Simulador: veja o passo a passo da execução.',
  ...
  },
  {
    id: 'history',
    title: 'Histórico',
    content: 'Histórico: acompanhe as etapas já executadas.',
  ...
  },
  {
    id: 'code',
    title: 'Código',
    content: 'Código gerado: copie ou baixe para estudar.',
  ...
  },
  {
    id: 'challenges',
    title: 'Desafios',
    content: 'Desafios: resolva exercícios e valide suas soluções.',
    selector:
      '[data-tour="challenges"], #nav-challenges, .nav-challenges, a[href*=\"challenges\"]',
    placement: 'top',
  },
];
...
function computePopoverRectForPlacement(
  targetRect,
  placement,
  popW,
  popH,
  ...
  return {
    ...
    width: popW,
    height: popH,
    topPx: `${Math.round(top)}px`,
    leftPx: `${Math.round(left)}px`,
  };
}
...
function chooseBestPlacement(targetRect, preferred, popW, popH) {
  ...
  for (const p of order) {
    ...
    const popRect = {
      left: pop.left - window.scrollX,
      top: pop.top - window.scrollY,
      width: pop.width,
      height: pop.height,
    };
    const tgtRectViewport = {
    ...
    const overlapX = Math.max(
      0,
      Math.min(
        popRect.left + popRect.width,
        tgtRectViewport.left + tgtRectViewport.width
      ) - Math.max(popRect.left, tgtRectViewport.left)
    );
    const overlapY = Math.max(
      0,
      Math.min(
        popRect.top + popRect.height,
        tgtRectViewport.top + tgtRectViewport.height
      ) - Math.max(popRect.top, tgtRectViewport.top)
    );
  ...
  if (best) {
    ...
    const left = Math.min(
      Math.max(pop.left + dx, 8),
      window.innerWidth - pop.width - 8
    );
    const top = Math.min(
      Math.max(pop.top + dy, 8),
      window.innerHeight - pop.height - 8
    );
    return {
      placement: p,
      style: {
  ...
  return {
    placement: preferred,
    style: {
      top: `${Math.round(window.innerHeight / 2 - popH / 2)}px`,
      left: `${Math.round(window.innerWidth / 2 - popW / 2)}px`,
      width: `${popW}px`,
      position: 'absolute',
    },
  };
...
export default function GuidedTourModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      const el = await findElementWithRetries(stepObj.selector, {
        retries: 12,
        delay: 200,
      });
      if (cancelled) return;
      ...
      const best = chooseBestPlacement(
        rect,
        stepObj.placement || 'bottom',
        popW,
        popH
      );
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: 12,
                  padding: 14,
                  boxShadow: '0 10px 40px rgba(2,6,23,0.16)',
                }}
              >
                <div
                  style={{
                ...
                >
                  <div style={{ fontWeight: 700, ...theme?.typography?.h3 }}>
                    {steps[index]?.title}
                  </div>
