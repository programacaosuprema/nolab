export function validateWorkspace(workspace) {
  ...
  workspace.getAllBlocks(false).forEach((block) => {
    ...
    // Variáveis declaradas
      ...
      const variable = block.getInputTargetBlock('VARIABLE');

      name = variable?.getFieldValue('VAR');
    }
    ...
    if (identifiers.has(name)) {
      const firstBlockId = identifiers.get(name);

      errors.push({
        blockId: block.id,
        firstBlockId,
        name,
        message: `Identificador duplicado: "${name}"`,
      });
