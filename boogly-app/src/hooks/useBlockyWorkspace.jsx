...
export function useBlocklyWorkspace({
  ...
  const tryGenerateC = useCallback(
    (ws, structure) => {
      try {
        if (!generateC) return '';
  ...
  useEffect(() => {
    try {
      ...
      if (!Blockly) {
        console.error(
          'Blockly não encontrado em window.Blockly — verifique import / script.'
        );
        setInitError(true);
        return;
      }
              ...
              if (JSGen) {
                try {
                  ...
                  if (typeof JSGen.workspaceToCode === 'function') {
                    ...
                    typeof window.Blockly.JavaScript.workspaceToCode ===
                      'function'
                  ) {
                ...
                console.warn(
                  'Nenhum generator JS disponível (javascriptGenerator ou window.Blockly.JavaScript)'
                );
                js = '';
              }
